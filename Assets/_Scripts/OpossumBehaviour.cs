...
public class OpossumBehaviour : MonoBehaviour
{
    ...
    private void FireBullet()
    {
        ...
        if (Time.frameCount % FireDelay == 0 && BulletManager.Instance().HasBullets())
        {
            ...
            BulletManager.Instance().GetBullet(BulletSpawnPoint.position, firingDirection);
            Debug.Log("Fired");
        }
    }
    ...
    {
        if (LOScomponent.colliders.Count > 0)
        {
            if (LOScomponent.collideWith.gameObject.name == "Player" || LOScomponent.colliders[0].gameObject.name == "Player")
            {
                return true;
            }
        }
